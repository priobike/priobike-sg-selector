...
class Hit(models.Model):
    """
    A model to persist true positives, false positives, and false negatives for LSAs.
    """

    run = models.ForeignKey(Run, on_delete=models.CASCADE)
    analysis = models.ForeignKey(RouteAnalysis, on_delete=models.CASCADE, related_name="hits")
    lsa = models.ForeignKey(LSA, on_delete=models.CASCADE, related_name="hits")

    # The key is used to denote the kind of hit, i.e. "tp", "fp", ...
    key = models.TextField()

    def __str__(self):
        return f"{self.analysis.route.id}-{self.lsa.id}"
