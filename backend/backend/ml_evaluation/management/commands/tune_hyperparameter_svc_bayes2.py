...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # define search space
        parameter_space = {
            'C': hp.loguniform('C', 1e-6, 100.0),
            'gamma': hp.loguniform('gamma', 1e-6, 100.0),
        }
        scoring = 'f1'
        # define evaluation
        ...
        best = fmin(fn=objective, space=parameter_space,
                    algo=tpe.suggest, max_evals=1000, trials=bayes_trials)
        ...
        mlp = SVC(
            C=space_eval(parameter_space, best)['C'],
            gamma=space_eval(parameter_space, best)['gamma']).fit(X_train, y_train)
