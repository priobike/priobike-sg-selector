...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, shuffle=config_train[config_train_id]["shuffle"], random_state=random_state)
        ...
        parameter_space = {
            ...
            'alpha': hp.choice('alpha', [0.0001, 0.05, 0.1, 1]),
        ...
        def objective(params):
            svc = MLPClassifier(max_iter=10000, **params)
            scores = cross_val_score(svc, X_train, y_train, cv=cv, scoring=scoring, n_jobs=-1)   
        ...
        bayes_trials = Trials()
        # Optimize
        best = fmin(fn = objective, space = parameter_space, algo = tpe.suggest, max_evals = 1000, trials = bayes_trials)
        ...
        mlp = MLPClassifier(
            hidden_layer_sizes=space_eval(parameter_space, best)['hidden_layer_sizes'],
            activation=space_eval(parameter_space, best)['activation'],
            solver=space_eval(parameter_space, best)['solver'],
            alpha=space_eval(parameter_space, best)['alpha'],
            learning_rate=space_eval(parameter_space, best)['learning_rate'],).fit(X_train,y_train)
        # Print the best f1 score for the testing dataset
        print(f'The f1 score for the testing dataset is {mlp.score(X_test, y_test):.8f}')