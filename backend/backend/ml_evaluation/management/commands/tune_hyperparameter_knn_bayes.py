...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        parameter_space = {
            'leaf_size': hp.choice('leaf_size', np.arange(1, 50, dtype=int)),
            'n_neighbors': hp.choice('n_neighbors', np.arange(1, 30, dtype=int)),
            'p': hp.choice('p', [1, 2]),
        ...
        def objective(params):
            svc = KNeighborsClassifier(**params)
        ...
        mlp = KNeighborsClassifier(
            leaf_size=space_eval(parameter_space, best)['leaf_size'],
            n_neighbors=space_eval(parameter_space, best)['n_neighbors'],
            p=space_eval(parameter_space, best)['p'],
            weights=space_eval(parameter_space, best)['weights'],
            algorithm=space_eval(parameter_space, best)['algorithm'],).fit(X_train, y_train)
