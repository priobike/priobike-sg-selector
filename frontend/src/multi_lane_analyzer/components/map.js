...
const bearing = new URLSearchParams(window.location.search).get('bearing');
...
export default class LSAMap extends React.Component {
  ...
  matchSignalgroups = () => {
    ...
    fetch(`${domain}:${port}/multi_lane_analyzer/api/route/${routeId}/matches${distance ? "?distance=" + distance : ""}${bearing ? "&bearing=" + bearing : ""}`)
    .then(res => res.json())
    .then(res => {
      this.setState({
        matches: res,
